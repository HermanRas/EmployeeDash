<?php
        ...
        $db = odbc_connect($connection_string, "EAIEmployeeUpdate", "EAIEmployeeUpdate") or die ("could not connect<br />");
            //run sql query
            $stmt = "select [EAI_PeopleUpdate].[dbo].[v3GDataReportToVIPDetail].*
            ...
            {  
                // Get row data
                        ...
                        echo('<td class="text-right">'. odbc_result($result, 'MB') .' MB </td>');
                        echo('<td class="text-right">R ' . sprintf("%01.2f",odbc_result($result, 'PricePerMeg'))  . '</td>');   
                        echo('</tr>');
                }
                echo ('</tbody>');
                echo ('</table>');
